...
export const NFTMarketplaceProvider = ({ children }) => {
    ...
    const requestNetworkSwitch = async () => {
        try {
            ...
            addToast(
                'Successfully switched to Polygon Amoy Testnet!',
                'success',
                3000
            );
    ...
    const checkIfWalletConnected = async () => {
        try {
            ...
            if (accounts.length) {
                ...
                addToast(
                    'No account found. Please connect your wallet.',
                    'info',
                    5000
                );
            }
            ...
            addToast(
                'Unable to connect to wallet. Using sample data.',
                'warning',
                5000
            );
    ...
    const createNFT = async (
        ...
        try {
            ...
            addToast(
                'Error while creating NFT. Please try again.',
                'error',
                5000
            );
    ...
    const createSale = async (url, formInputPrice, isReselling, id) => {
        ...
        try {
            ...
            if (!isOnCorrectNetwork) {
                console.warn(
                    '⚠️ Wrong network detected! Chain ID:',
                    currentChainId
                );
                addToast(
            ...
            if (error.code === 'CALL_EXCEPTION') {
                ...
                addToast(
                    "Contract call failed. Please ensure you're on Polygon Amoy Testnet.",
                    'error',
                    6000
                );
                ...
                addToast(
                    'Error while creating sale. Please try again.',
                    'error',
                    5000
                );
            }
    ...
    const fetchNFTs = async () => {
        ...
        try {
            ...
            addToast(
                'Unable to fetch NFTs from blockchain. Using sample data.',
                'warning',
                6000
            );
            console.log(error);
    ...
    const fetchMyNFTsOrListedNFTs = async (type) => {
        ...
        try {
            ...
            addToast(
                'Unable to fetch your NFTs from blockchain.',
                'info',
                5000
            );
    ...
    const fetchNFTsByAddressFromURL = async () => {
        try {
                        ...
                        async ({
                            ...
                            try {
                                ...
                                if (tokenURI) {
                                    ...
                                    console.log(
                                        'TokenURI is undefined for tokenId:',
                                        tokenId
                                    );
                                }
            ...
            addToast(
                'Error while fetching NFTs. Please try again.',
                'error',
                5000
            );
    ...
    const buyNFT = async (nft) => {
        ...
        try {
            ...
            if (!currentAccount) {
                addToast(
                    'Please connect your wallet before buying an NFT',
                    'warning',
                    5000
                );
            ...
            if (!contract) {
                addToast(
                    'Failed to connect to smart contract. Please try again.',
                    'error',
                    5000
                );
            ...
            if (error.message?.includes('user rejected')) {
                ...
                error.message?.includes('Network switch required')
            ) {
                // Already handled above
            } else {
                addToast(
                    'Error while buying NFT. Please try again.',
                    'error',
                    5000
                );
            }
    ...
    const placeBid = async (tokenId, bidAmount) => {
        try {
            ...
            const formattedBidAmount = ethers.utils.parseUnits(
                bidAmount.toString(),
                'ether'
            );
    ...
    const fetchBidsForNFT = async (tokenId) => {
        try {
            ...
            console.log(
                'Fetched top 7 bids in fetchTop7BidsForNFT:',
                formattedTop7Bids
            );
    ...
    const acceptBid = async (bidId) => {
        try {
            ...
            if (receipt.status === 1) {
                ...
                addToast(
                    'Error accepting bid. Transaction failed.',
                    'error',
                    5000
                );
            }
    ...
    const leaveSellerReview = async (sellerAddress, reviewText) => {
        try {
            ...
            console.log(
                'Calling addSellerReview function with parameters:',
                sellerAddress,
                reviewText
            );
            const transaction = await contract.addSellerReview(
                sellerAddress,
                reviewText
            );
    ...
    const fetchReviewsForAddress = async (sellerAddress) => {
        try {
            ...
            console.log(
                'Calling fetchReviewsForAddress function with parameter:',
                sellerAddress
            );
            const reviews = await contract.fetchReviewsForAddress(
                sellerAddress
            );
            ...
            // Log any unexpected errors
            console.error(
                'Error fetching reviews for address',
                sellerAddress,
                error
            );
    ...
    const fetchNFTsWithBids = async () => {
        try {
                ...
                marketItems.map(async (item) => {
                    ...
                    return {
                        ...
                        currentBidder:
                            activeBids.length > 0
                                ? activeBids[0].bidder
                                : 'No active bids', // Use the first active bid to define the current highest bidder
                        owner: item.owner,
                        seller: item.seller,
                        sold: item.sold,
                        ...additionalData,
            ...
            const itemsWithActiveBids = itemsWithBids.filter(
                (item) => item.currentBidder !== 'No active bids'
            );
